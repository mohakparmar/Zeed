...
struct IGMockLoader {
    ...
    static func loadMockFile(named fileName:String, andExt ext: String, bundle:Bundle = .main) throws -> IGStories {
        guard let url = bundle.url(forResource: fileName, withExtension: ext) else {throw MockLoaderError.invalidFileName(fileName)}
        do {
