...
- (UIView*)createMessageWithText: (NSString*)text Image: (NSString*)strUrl DateTime: (NSString*)dateTimeString isReceived: (BOOL)isReceived isVideo: (BOOL)IsVideoBool type:(NSString *)type
{
    ...
    chatBubbleView.backgroundColor = [self setColor:isReceived ? @"707070" : @"007DA5" alpha:1];
...
//    chatBubbleContentView.backgroundColor = [UIColor whiteColor];
    ...
    if (dateTimeString != nil)
    {
        ...
        chatTimeLabel.textColor = [UIColor whiteColor];
    ...
    if (strUrl.length) {
        ...
        NSString *str = strUrl;
        NSURL *imgUrl = [NSURL URLWithString:[NSString stringWithFormat:@"%@",str]];

        if ([type.uppercaseString containsString:@"AUDIO"]) {
            chatBubbleImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 26, maxBubbleWidth-30, 50)];
            [chatBubbleContentView addSubview:chatBubbleImageView];

            UIImageView *imgPlayButton = [[UIImageView alloc] initWithFrame:CGRectMake(10, 36, 40, 40)];
            imgPlayButton.image = [UIImage imageNamed:@"play.circle.fill"];
            imgPlayButton.tintColor = [self setColor:@"65FECB" alpha:1.0];
            imgPlayButton.tag = 1;

            UIView *viewProgress = [[UIView alloc] initWithFrame:CGRectMake(65, 60, maxBubbleWidth-110, 6)];
            viewProgress.backgroundColor = [UIColor darkGrayColor];
            viewProgress.layer.cornerRadius = 3;
            [chatBubbleContentView addSubview:viewProgress];
            
            [chatBubbleContentView addSubview:imgPlayButton];
            
            UILabel *lblAudio = [[UILabel alloc] initWithFrame:CGRectMake(65, 30, 100, 30)];
            lblAudio.text = @"Audio File";
            lblAudio.textColor = [UIColor whiteColor];
            [chatBubbleContentView addSubview:lblAudio];

            
        } else {
            if ([str.uppercaseString containsString:@"GIF"]) {
               [chatBubbleImageView sd_setImageWithURL:imgUrl];
            }  else {
                if (IsVideoBool == YES) {
                    AVURLAsset *asset = [[AVURLAsset alloc] initWithURL:imgUrl options:nil];
                    ...
                    chatBubbleImageView.image = FrameImage;
                } else {
                    [chatBubbleImageView sd_setImageWithURL:imgUrl];
                }
            }
    ...
    UIImage *image = [[UIImage imageNamed:@"chat_arrow"] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
    arrowIV.image = image;
    [arrowIV setTintColor:[self setColor:isReceived ? @"707070" : @"007DA5" alpha:1]];
...
- (UIView*)createCommentWithText: (NSString*)text Image: (NSString*)strUrl DateTime: (NSString*)dateTimeString isReceived: (BOOL)isReceived isVideo:(BOOL)IsVideoBool userName:(NSString *)strUserName userImage:(NSString *)strUserImage {
